...
export const ContactFormSchemaCarrier = z
  .object({
    company_name: z.string().min(2, { message: "Company name is required" }),
    company_street_1: z.string().optional(),
    company_street_2: z.string().optional(),
...
export const BasicSipTrunkSchema = z.object({
  ...
  block_matching_src_dst: z
    .string()
    .min(1, { message: "Block matching scr/dst is required" }),
});
...
export const rateDeckUploadBaseSchema = z.object({
  // effective_date: z.date({
  //   required_error: "Effective date is required",
  // }),
});

export const rateDeckUploadWithSipTrunkSchema = rateDeckUploadBaseSchema.extend(
  {
    sip_trunk_id: z.string().min(1, { message: "SIP Trunk is required" }),
    effective_date: z.date({
      required_error: "Effective date is required",
    }),
  }
...
export const AddUser = z.object({
  user_name: z.string().min(1, { message: "Group name is required" }),
...
export const RateDeckForm = z
  .object({
    ...
    populate_interminate_as: z
      .string()
      .min(1, { message: "Populate indeterminate as is required" }),
    rounding_percision: z.coerce
      .number()
      .min(1, { message: "Rounding precision is required" }),
    rounding_method: z
      .string()
      .min(1, { message: "Rounding method is required" }),
    ...
    non_us_canada_country_code_1: z.boolean({
    ...
    if (data.include_toll_free === true) {
      if (data.toll_free_price === undefined || data.toll_free_price < 0.01) {
        ctx.addIssue({
          ...
          message:
            "Toll-free price is required when include toll-free is checked",
