...
export default function SpecificCustomer() {
  ...
  <div className="flex flex-col md:flex-row items-center md:justify-between mb-6">
    <div className="flex items-center gap-4">
      ...
      <div>
        <h2 className="text-2xl font-semibold">{data?.account?.company_name}</h2>
        <Badge
          variant={
            data?.user?.[0]?.is_active === true ? "success" : "destructive"
          }
        >
      ...
      <TabsCommon TABS={CUSTOMER_LIST_TABS} DEFAULT_TAB={state?.activeTab ? state?.activeTab : CUSTOMER_LIST_TABS[0].value}  />
    </div>
  );
}
